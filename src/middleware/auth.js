...
const hydrateChatLocals = async (res, user) => {
...
const verifyToken = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const users = await queryAsync("SELECT status, deleted_at FROM users WHERE id = ? LIMIT 1", [decoded.id]);
      ...
      await hydrateChatLocals(res, decoded);
      return next();
    } catch (err) {
      clearAuthCookie(res);
