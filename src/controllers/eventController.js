...
const buildStoredMediaPath = (filename) => `storage/uploads/${filename}`;
...
const resolveStoredMediaPath = (filePath) => {
    ...
    try {
        ...
        const eventId = await db.withTransaction(async (tx) => {
            ...
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                const relativePath = buildStoredMediaPath(file.filename);
...
exports.deleteEvent = async (req, res) => {
    try {
        ...
        media.forEach(item => {
            const absolutePath = resolveStoredMediaPath(item.file_path);
            if (absolutePath) {
                try {
