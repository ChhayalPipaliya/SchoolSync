...
const groupAdminController = {
        ...
        try {
            ...
            await withTransaction(async ({ execute }) => {
                const result = await execute(
                    ...
                    [ cleanFirstName, cleanLastName, cleanEmail, cleanPhone, hashedPassword, cleanStatus, ]
                );
            ...
            await withTransaction(async ({ execute, query }) => {
                if (password && password.trim()) {
                    ...
                    await execute(
                        ...
                        [ cleanFirstName, cleanLastName, cleanEmail, cleanPhone, hashedPassword, cleanStatus, req.params.id, ]
                    );
                } else {
                    await execute(
                        `UPDATE users SET school_id = NULL, first_name = ?, last_name = ?, email = ?, phone = ?, status = ? WHERE id = ? AND role = 'group_admin'`,
                        [ cleanFirstName, cleanLastName, cleanEmail, cleanPhone, cleanStatus, req.params.id, ]
                    );
                };
    ...
    delete: async (req, res) => {
        try {
            ...
            await executeAsync("UPDATE users SET deleted_at = NOW(), status = 'inactive' WHERE id = ? AND role = 'group_admin'", [adminId]);
            req.flash("success", "Group admin deleted successfully.");
            res.redirect("/superadmin/group-admins");
        } catch (error) {
            console.error("Group Admin Delete Error:", error);
            req.flash("error", "Failed to delete group admin.");
            res.redirect("/superadmin/group-admins");
        };
    },
};
