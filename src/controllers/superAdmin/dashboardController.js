...
// Helper to broadcast real-time updates via Socket.io
const broadcastDashboardUpdates = async () => {
  try {
    ...
    try {
      ...
      // socket.io not initialized yet or not running
      return;
    }
    if (!io) return;
...
const getSchoolLeaderboard = async () => {
  const sql = `
    SELECT 
      ...
      ROUND(
        ...
        20 + -- Recent activity (default 20)
        15 + -- Teacher activity (default 15)
  ...
  // Group leaderboard entries for different tabs
...
const compileDashboardData = async (filters = {}) => {
  const [metrics, health, leaderboard, trend, planDistribution, planIntelligence] = await Promise.all([
...
const dashboardController = {
  getDashboard: async (req, res) => {
    try {
      ...
      // Initialize periodic socket broadcasts if not already initialized
      initDashboardSocketInterval();

      // Compile current live dashboard data
  ...
  resolveAlert: async (req, res) => {
    try {
      const { id } = req.params;
      // Extract numeric ID from alerts if prefixed (e.g. backup_delay-11)
