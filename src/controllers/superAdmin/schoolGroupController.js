...
const schoolGroupController = {
    ...
    create: async (req, res) => {
        try {
            ...
            await executeAsync(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [ group_name.trim(), owner_name || null, email || null, phone || null, city || null, address || null, status || "active", req.user.id ]
            );
    ...
    update: async (req, res) => {
        try {
            ...
            await executeAsync(
                ...
                WHERE id = ?`,
                [ group_name.trim(), owner_name || null, email || null, phone || null, city || null, address || null, status || "active", req.user.id, req.params.id ]
            );
    ...
    delete: async (req, res) => {
        try {
            ...
            const schools = await queryAsync("SELECT id FROM schools WHERE school_group_id = ?", [groupId]);
            if (schools && schools.length > 0) {
                req.flash("error", "Cannot delete school group because it has linked schools/branches. Please unlink them first.");
                return res.redirect("/superadmin/school-groups");
            };
            await executeAsync("DELETE FROM school_groups WHERE id = ?", [groupId]);
            req.flash("success", "School group deleted successfully.");
            res.redirect("/superadmin/school-groups");
        } catch (error) {
            console.error("School Group Delete Error:", error);
            req.flash("error", "Failed to delete school group.");
            res.redirect("/superadmin/school-groups");
        };
    },
};
