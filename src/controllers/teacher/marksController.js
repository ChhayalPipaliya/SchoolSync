...
function calculateGrade(marksObtained, maxMarks) {
  if (!marksObtained || !maxMarks || maxMarks === 0) return { grade: 'E', gradePoint: 0.0 };
...
function isPassed(marksObtained, maxMarks) {
  return (parseFloat(marksObtained) / parseFloat(maxMarks)) * 100 >= 33;
}

async function getTeacher(userId, schoolId) {
  const [rows] = await db.execute(
    'SELECT * FROM teachers WHERE user_id = ? AND school_id = ? LIMIT 1',
    [userId, schoolId]
  );
  return rows[0] || null;
}
...
exports.getEnterMarks = async (req, res) => {
  try {
    ...
    const teacher = await getTeacher(user.id, schoolId);
    if (!teacher) {
    ...
    if (examId && classId) {
      if (!await teacherPermissions.checkTeacherClassAccess(teacher.id, schoolId, classId, subjectId, { allowGeneralSubject: !subjectId })) {
...
exports.postEnterMarks = async (req, res) => {
  ...
  try {
    ...
    if (!teacher || !await teacherPermissions.checkTeacherClassAccess(teacher.id, schoolId, class_id, subject_id, { allowGeneralSubject: false })) {
      req.flash('error', 'You are not assigned to this class.');
      return res.redirect('/teacher/marks');
    }
    ...
    for (const [studentKey, data] of Object.entries(marks)) {
      ...
      const status = isPassed(marksObtained, exam.max_marks) ? 'pass' : 'fail';
...
exports.getResultAnalysis = async (req, res) => {
  try {
    ...
    const [exams] = await db.query('SELECT * FROM exams WHERE school_id = ? ORDER BY start_date DESC', [schoolId]);
    ...
    if (examId && classId) {
      if (!await teacherPermissions.checkTeacherClassAccess(teacher.id, schoolId, classId)) {
        req.flash('error', 'You are not assigned to this class.');
