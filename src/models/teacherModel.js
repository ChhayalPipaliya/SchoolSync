...
class TeacherModel {    
    async createTeacher(data) {
        const [result] = await db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [data.school_id, data.user_id, data.subject, data.qualification, data.experience,
             data.gender, data.dob, data.marital_status, data.father_name, data.mother_name,
             data.current_address, data.permanent_address, data.joining_date]
        );
    ...
    async updateTeacher(teacherId, data) {
        ...
        const allowedFields = ['subject', 'qualification', 'experience', 'gender', 'dob', 
                              'marital_status', 'father_name', 'mother_name', 
                              'current_address', 'permanent_address', 'joining_date', 'emergency_contact'];
    ...
    async getDashboardStats(teacherId, schoolId) {
        const [[teacher]] = await db.execute('SELECT user_id FROM teachers WHERE id = ?', [teacherId]);
        const userId = teacher ? teacher.user_id : null;
        ...
        const [[attendance]] = await db.execute(
            `SELECT COUNT(*) as count FROM attendance 
             WHERE marked_by = ? AND date = CURDATE()`,
            [userId]
        );
