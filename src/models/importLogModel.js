...
class ImportLogModel {
    async createLog(data) {
        const result = await db.executeAsync(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                data.school_id, data.imported_by, data.user_role, data.entity_type,
                data.file_name, data.file_path, data.total_rows || 0, data.success_count || 0,
                data.failed_count || 0, data.error_report_path || null, data.status || 'processing'
            ]
        );
