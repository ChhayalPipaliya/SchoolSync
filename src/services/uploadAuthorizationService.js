            ...
            ? `(
                EXISTS (
                    ...
                    WHERE own_student.id = fp.student_id AND own_student.school_id = fp.school_id
                        AND own_student.user_id = ? AND own_student.deleted_at IS NULL
                )
                OR EXISTS (
                    ...
                    WHERE fpa.payment_id = fp.id AND fpa.school_id = fp.school_id
                        AND allocated_student.user_id = ? AND allocated_student.deleted_at IS NULL
                )
                OR EXISTS (
                    ...
                    WHERE legacy_fee.payment_id = fp.id AND legacy_fee.school_id = fp.school_id
                        AND legacy_student.user_id = ? AND legacy_student.deleted_at IS NULL
                )
            )`
            : `(
                EXISTS (
