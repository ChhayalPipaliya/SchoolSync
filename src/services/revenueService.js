...
const revenueService = {
   ...
   * Calculates from subscription_payments status='completed' (and/or 'paid') in current month
   */
  getMRR: async () => {
    const sql = `
  ...
  /**
   * Calculates revenue growth % (vs last month)
   */
  ...
  /**
   * Calculates Average Revenue Per School (ARPS)
   */
  ...
  /**
   * Calculates Revenue Forecast (next quarter projection)
   */
  ...
  /**
   * Generates 12-Month Revenue Trend data
   */
  getRevenueTrend: async () => {
    ...
    // Get actual database revenue for the last 12 months
    const sql = `
      SELECT 
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      if (dbRevenueMap.has(monthStr)) {
        ...
        // Fallback to baseline progression for visual completeness only if there is some revenue
  ...
  /**
   * Plan revenue distribution for Doughnut chart
   */
