    ...
    return db.withTransaction(async ({ execute, query }) => {
        ...
        for (const row of preview.rows) {
            await execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [batchId, row.student_id, row.from_academic_record_id, row.from_class_id, row.to_class_id, row.result_status, row.promotion_action, row.reason]
            );
        };
...
async function confirmPromotionBatch(batchId, schoolId, adminUserId, overrides = {}) {
    return db.withTransaction(async ({ query, execute }) => {
        ...
        const summary = { promoted: 0, repeated: 0, skipped: 0, graduated: 0, duplicateSkipped: 0 };
