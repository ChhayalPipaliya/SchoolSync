...
async function runSubscriptionExpiryReminderCheck() {
    ...
    try {
        const sql = `
            SELECT sub.id as sub_id, sub.school_id, sub.end_date, sub.status as sub_status, sch.school_name, sch.school_email, sch.principal_name, p.name as plan_name,
        ...
        for (const sub of expiringSubs) {
            ...
            const renewUrl = `${process.env.APP_URL || 'http://localhost:3000'}/schooladmin/subscription`;
...
async function runExpiredSubscriptionCheck() {
    ...
    try {
        ...
        const sql = `
            SELECT sub.id as sub_id, sub.school_id, sub.end_date, sch.school_name, sch.school_email, sch.principal_name, p.name as plan_name
            FROM subscriptions sub
            JOIN schools sch ON sub.school_id = sch.id
            JOIN plans p ON sub.plan_id = p.id
            WHERE sub.status IN ('active', 'trial')
        ...
        for (const sub of expiredSubs) {
            await db.withTransaction(async ({ execute }) => {
                ...
                await execute(
                    "UPDATE schools SET status = 'expired', updated_at = NOW() WHERE id = ?",
                    [sub.school_id]
                );
...
module.exports = {
    ...
    runExpiredSubscriptionCheck,
    runQuotaUsageMonitoringCheck
};
